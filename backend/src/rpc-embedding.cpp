...
static void batch_decode(llama_context * ctx, llama_batch & batch, float * output, int n_seq, int n_embd_out, int embd_norm, std::vector<std::string> prompts) {
    ...
    for (int i = 0; i < batch.n_tokens; i++) {
        ...
        if (old_embd_pos < embd_pos)
        {
            embeddings.emplace_back(out, out + n_embd_out);
            old_embd_pos = embd_pos;
        }
    }
}
...
static std::vector<std::vector<float>> generate(std::vector<std::string> prompts) {
    ...
    int argc_fake = static_cast<int>(argv_vec.size() - 1);
    char** argv_fake = argv_vec.data();

    if (!common_params_parse(argc_fake, argv_fake, params, LLAMA_EXAMPLE_EMBEDDING)) {
        return {};
    }
    ...
    std::vector<float> flat_embeddings(n_embd_count * n_embd_out, 0);
    float * emb = flat_embeddings.data();
    ...
    for (int k = 0; k < n_prompts; k++) {
        ...
        if (batch.n_tokens + n_toks > n_batch || s >= n_seq_max) {
            ...
            batch_decode(ctx, batch, out, s, n_embd_out, params.embd_normalize, prompts);
    ...
    if (params.embd_out == "json" || params.embd_out == "json+" || params.embd_out == "array") {
        ...
        std:: cout << "Por aquí está pasando" << std::endl;
    }

    LOG("\n");
    llama_perf_context_print(ctx);

    // build nested vector: one vector<float> per embedding
    std::vector<std::vector<float>> results;
    results.reserve(n_embd_count);
    for (int j = 0; j < n_embd_count; ++j) {
        results.emplace_back(emb + j * n_embd_out, emb + (j + 1) * n_embd_out);
    }
